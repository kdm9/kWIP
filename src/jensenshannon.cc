...
distance(khmer::CountingHash &a, khmer::CountingHash &b)
{
    ...
    for (size_t tab = 0; tab < 1; tab++) {
        ...
        for (size_t bin = 0; bin < tablesizes[tab]; bin++) {
            ...
            _small_log_offset = 1.0 / (float)a_sum;

            float total = a_freq + b_freq + _small_log_offset;

            float a_log = a_freq * log((a_freq + _small_log_offset)/total);
            float b_log = b_freq * log((b_freq + _small_log_offset)/total);
